...
const ShimmerCircles = () => {
      const circleRefs = useRef([]);

      useEffect(() => {
            let xOffsets = shimmerData.map(d => d.x);
            let yOffsets = shimmerData.map(d => d.y);
            ...
            const interval = setInterval(() => {
                  circleRefs.current.forEach((circle, i) => {
                        if (circle) {
                              ...
                              // 화면 밖으로 나가면 되돌리기
                              if (xOffsets[i] > 1000) xOffsets[i] = -100;
                              if (yOffsets[i] > 300) yOffsets[i] = -100;

                              circle.setAttribute("cx", xOffsets[i].toString());
                              circle.setAttribute("cy", yOffsets[i].toString());
                        }
                  });
            }, 30);
...
const TitleText = () => {
      return (
            <div className="text_box text-effect-wrapper">
                  <h2 className="text h1">
                        <div className="headline">
                              <svg viewBox="0 0 900 270" xmlns="http://www.w3.org/2000/svg" className="headline-svg">
                                    <defs>
                                          <linearGradient id="grad" x1="0%" y1="0%" x2="200%" y2="0%">
                                                <stop offset="0%" stopColor="#00D5E0" />
                                                <stop offset="25%" stopColor="#69f6ff" />
                                                <stop offset="50%" stopColor="#0088A0" />
                                          </linearGradient>
                                          ...
                                          <mask id="strokeMask">
                                                ...
                                                <use href="#textPath" fill="none" stroke="white" strokeWidth="3" />
                                          </mask>
                                    </defs>

                                    <use href="#textPath" stroke="url(#grad)" strokeWidth="2" fill="none" />

                                    {/* 반짝이는 흰 원을 마스크 위에 덧씌움 */}
                                    <g mask="url(#strokeMask)">
                                          <ShimmerCircles />
                                    </g>

                                    {/* 실질 경로 정의는 defs 밖에서 필요함 */}
                                    <path
                                          id="textPath"
                                          d="M81.9,93.9H61.4V55.7H21.2v38.2H0.7V0.7h20.5v36.7h40.2V0.7h20.5V93.9z M167.5,93.9H99V0.7h66.8V18h-46.5v20.2
